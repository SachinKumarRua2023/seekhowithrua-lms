...
## PRIORITY 12 — Google OAuth (Login with Google)
# File: django-react-ml-app/backend/ + frontend/

### Backend: install and configure allauth
...
Add views.py Google login view:
```python
...
class GoogleLogin(SocialLoginView):
    ...
    callback_url    = 'https://app.seekhowithrua.com/auth/google/callback'
...
### Frontend: Google OAuth button in VCRoom + main app
...
Wrap your app root with provider:
```jsx
...
Add Google login button to auth screen in VCRoom.jsx:
```jsx
import { GoogleLogin } from '@react-oauth/google';

// Add inside the auth card, below the existing login button:
<div style={{ margin: '14px 0', textAlign: 'center', color: 'var(--muted)', fontSize: '0.75rem' }}>— OR —</div>
<GoogleLogin
  onSuccess={async (credentialResponse) => {
    ...
    if (ok && data.key) {
      ...
      const prof = await apiFetch('/api/profile/');
      if (prof.ok) {
        const role = prof.data?.profile?.role || 'learner';
        setUser({ id: prof.data.id, name: prof.data.username || prof.data.email.split('@')[0], role, email: prof.data.email });
        localStorage.setItem('cosmos_user', JSON.stringify(prof.data));
        push('Welcome! Signed in with Google.', 'cyan');
      }
    }
  }}
  onError={() => push('Google login failed. Try email login.', 'err')}
  text="signin_with"
  shape="pill"
  theme="filled_black"
  size="large"
...
Google Cloud Console setup:
  ...
  2. OAuth consent screen → External → add seekhowithrua.com
  3. Credentials → OAuth 2.0 Client → Web Application
  4. Authorised origins: https://app.seekhowithrua.com
  5. Authorised redirect URIs: https://app.seekhowithrua.com/auth/google/callback
  6. Copy Client ID + Secret → paste into Django settings

---

## PRIORITY 13 — Payment Gateway (Razorpay)
# Handles: Rs149/month subscriptions, Rs2000 host activation, Rs999 B2B plan

### Backend: install Razorpay
...
PLANS = {
    'audience_monthly': { 'amount': 14900,  'currency': 'INR', 'name': 'Panel Audience — Monthly' },
    'host_activation':  { 'amount': 200000, 'currency': 'INR', 'name': 'Host Activation — One Time' },
    'creator_pro':      { 'amount': 49900,  'currency': 'INR', 'name': 'Creator Pro — Monthly' },
    'b2b_enterprise':   { 'amount': 99900,  'currency': 'INR', 'name': 'B2B Enterprise — Monthly' },
}
...
def create_order(request):
    ...
    order = client.order.create({
        ...
        'notes':    { 'user_id': str(request.user.id), 'plan': plan_id }
    })
    return Response({
        'order_id':  order['id'],
        'amount':    order['amount'],
...
def verify_payment(request):
    ...
    # Activate the plan for this user
    activate_plan(request.user, plan_id)
    return Response({'status': 'success', 'plan': plan_id})

def activate_plan(user, plan_id):
    from voice_rooms.models import UserProfile
    profile, _ = UserProfile.objects.get_or_create(user=user)
    if plan_id == 'host_activation':
        profile.can_host = True
    elif plan_id in ('audience_monthly', 'creator_pro'):
        profile.subscription_active = True
    profile.save()
```

Add to payments/urls.py:
```python
from django.urls import path
from . import views
urlpatterns = [
...
### Frontend: Razorpay checkout in VCRoom + main app
...
async function handlePayment(planId) {
  ...
  if (!ok) { push('Payment init failed', 'err'); return; }
  ...
  const rzp = new window.Razorpay({
    ...
    handler: async (response) => {
      ...
      if (verify.ok) {
        push('Payment successful! Access unlocked.', 'cyan');
        // Refresh user profile to reflect new subscription
        const prof = await apiFetch('/api/profile/');
        if (prof.ok) localStorage.setItem('cosmos_user', JSON.stringify(prof.data));
      }
    },
  });
  rzp.open();
...
Add to UserProfile model (voice_rooms/models.py):
```python
...
## PRIORITY 14 — Panel Ad Monetisation
# How ads show inside voice rooms for 1K+ follower hosts

### Architecture:
  Host with 1000+ followers creates a panel → ad slot unlocked
  Ad shows as a banner in the sidebar during the panel session
  Ad served by Google AdSense (easiest) or direct brand sponsorship

### Google AdSense integration (simplest):
```jsx
// In VCRoom.jsx sidebar — show only when panel is live and host has 1K+ followers
{panelInfo?.hostFollowers >= 1000 && (
  <div className="ad-slot" style={{ padding: 12, borderTop: '1px solid var(--border)' }}>
    <div style={{ fontSize: 9, color: 'var(--muted)', marginBottom: 6, letterSpacing: 2 }}>SPONSORED</div>
...
Load AdSense script in index.html:
```html
<script async src="https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-XXXXXXXXXXXXXXXX" crossorigin="anonymous"></script>
```

### Revenue split model:
  Google pays per impression/click to seekhowithrua.com AdSense account
  Platform keeps 30%, pays 70% to host creator monthly
  Payment via NEFT to host bank account (collected at KYC onboarding at 1K followers)

### Host follower count in panel API:
Add to /api/panels/<id>/join/ response:
```python
return Response({
    ...existing_response,
    'host_followers': panel.created_by.followers.count(),
    'host_can_monetise': panel.created_by.followers.count() >= 1000,
})
```

---

## PRIORITY 15 — Security
# Non-negotiable before going public. All of these are simple additions.

### 1. HTTPS everywhere (already done via Vercel + Render)
  Vercel enforces HTTPS on all frontend deployments.
  Render enforces HTTPS on all backend deployments.
  No action needed.

### 2. Django security settings (add to settings.py):
```python
...
# Rate limiting (install: pip install djangorestframework-simplejwt django-ratelimit)
REST_FRAMEWORK = {
...
### 3. Input validation on all API views:
```python
# Add to all views that accept user input:
...
### 4. Token expiry (add to settings.py):
```python
...
TOKEN_EXPIRED_AFTER_SECONDS = 30 * 24 * 60 * 60
```
...
### 6. VCR moderation — auto-block abusive content:
```python
BLOCKED_WORDS = ['phone_number_pattern', 'email_pattern', 'explicit_terms']
import re
...
def check_message(text):
    ...
    if phone_pattern.search(text) or email_pattern.search(text):
        return False, 'Contact sharing not allowed in public rooms'
    return True, None
```

---

## PRIORITY 16 — CI/CD Pipeline
# Automatic testing + deployment on every git push

### Current setup (already working):
  git push → GitHub → Vercel auto-deploys frontend (seekhowithrua-seo, app)
  git push → GitHub → Render auto-deploys backend (django-react-ml-app)

### Add GitHub Actions for automated testing:
File: .github/workflows/ci.yml (create in seekhowithrua-seo repo)
...
jobs:
  ...
  backend-test:
    ...
    steps:
      ...
      - run: python manage.py check          # Django system check
...
### Branch strategy (use this):
  main    → production (Vercel + Render auto-deploy)
  dev     → staging (separate Vercel preview deployment)
  feature/* → PR required before merging to dev
  fix/*      → PR required before merging to dev

Rule: NEVER push directly to main. Always PR from dev → main.
This prevents breaking live users.

### Pre-commit hooks (install locally):
```bash
...
## COMPLETE UPDATED BUILD ORDER

Session 1 (NEXT):      VCR bug fix (Priority 1) + blog push (today)
Session 2:             Django voice_rooms models + recommendation engine (Priority 2)
Session 3:             API endpoints + migrate + deploy (Priority 3)
Session 4:             Google OAuth backend + frontend (Priority 12)
Session 5:             Razorpay payment gateway (Priority 13)
Session 6:             VCR follow + upvote + onboarding screen (Priority 4)
Session 7:             Leaderboard component (Priority 5) + subdomain deploy (Priority 6)
Session 8:             Security hardening (Priority 15) + CI/CD (Priority 16)
Session 9:             Panel ad monetisation (Priority 14)
Session 10:            memory-game.html (Priority 7)
Session 11:            data-science.html animations (Priority 8)
Session 12:            quiz-battle.html (Priority 7 pt2)
Session 13:            Mobile app core screens (Priority 11)
Ongoing:               1 blog post per day (Priority 10)
                       Internal links on chapter pages (Priority 9)
